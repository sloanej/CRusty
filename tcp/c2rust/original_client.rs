...
srand(time(0 as libc::c_int) as libc::c_uint);
...
unsafe extern "C" fn handler(mut _: libc::c_int) {
...
if sockfd == -(1 as libc::c_int) {
   printf(b"socket creation failed...\n\x00" as *const u8 as
              *const libc::c_char);
   exit(0 as libc::c_int);
} else {
...
if connect(sockfd, &mut servaddr as *mut sockaddr_in as *mut sockaddr,
          ::std::mem::size_of::<sockaddr_in>() as libc::c_ulong as
              socklen_t) != 0 as libc::c_int {
   printf(b"connection with the server failed...\n\x00" as *const u8 as
              *const libc::c_char);
   exit(0 as libc::c_int);
} else {
   printf(b"connected to the server..\n\x00" as *const u8 as
              *const libc::c_char);
}
// function for chat
func(sockfd);
